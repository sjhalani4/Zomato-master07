...
const MobileTabs = () =>{
    ...
    <div className="lg:hidden bg-white shadow-lg p-3 fixed bottom-0 z-10 w-full  flex justify-between text-gray-500 border md:justify-evenly">
        
            {
                allTypes.map((items)=>(
                    <Link to={`${items.id}`}>
        <div className={classnames("flex flex-col items-center text-xl" , {"flex flex-col items-center text-xl text-zomato-400 relative":type===items.id})}>
...
const LargeTabs = () =>{
    ...
    return (<>
        <div className="hidden lg:flex gap-14 container px-20 my-8 mx-auto ">
            ...
            {allTypes.map((items)=> (
                <Link to={`/${items.id}`}>
                    <div className={classnames("flex items-center gap-3",{"border-b-2 border-zomato-400 pb-2 transition duration-1000 ease-in-out":type===items.id})}>
                <div className={classnames("w-16 h-16 p-4 rounded-full bg-gray-100",{[`bg-${items.activeColor}-100`]:type===items.id}) }>
